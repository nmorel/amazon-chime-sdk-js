...
export default class SimulcastUplinkPolicyNScaleLowStream implements SimulcastUplinkPolicy {
  ...
  private optimalParameters: DefaultVideoCaptureAndEncodeParameter;
  private parametersInEffect: DefaultVideoCaptureAndEncodeParameter;
  ...
  private isContentAttendee: boolean = false;

  constructor(private selfAttendeeId: string, private logger: Logger) {
    this.optimalParameters = new DefaultVideoCaptureAndEncodeParameter(0, 0, 0, 0, true);
    this.parametersInEffect = new DefaultVideoCaptureAndEncodeParameter(0, 0, 0, 0, true);
    ...
    this.isContentAttendee = new DefaultModality(selfAttendeeId).hasModality(
  ...
  private calculateEncodingParameters(
    ...
    if (
      ...
      if (!this.isContentAttendee && this.shouldDisableSimulcast) {
  ...
  updateIndex(videoIndex: VideoStreamIndex): void {
    ...
    this.optimalParameters = new DefaultVideoCaptureAndEncodeParameter(
      this.captureWidth(),
      this.captureHeight(),
