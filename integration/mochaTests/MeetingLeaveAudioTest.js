...
class MeetingLeaveAudioTest extends SdkBaseTest {
  ...
  async runTestOnSingleSessions() {
    ...
    await test_window.runCommands(
      async () => await this.addUserToMeeting(test_attendee_id, session)
    );
    await monitor_window.runCommands(
      async () => await this.addUserToMeeting(monitor_attendee_id, session)
    );
    ...
    await monitor_window.runCommands(
      async () => await RemoteAudioCheck.executeStep(this, session, 'AUDIO_ON')
    );

    await test_window.runCommands(async () => await LeaveMeetingStep.executeStep(this, session));
    await monitor_window.runCommands(
      async () => await RemoteAudioCheck.executeStep(this, session, 'AUDIO_OFF')
    );
