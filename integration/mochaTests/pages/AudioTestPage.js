...
class AudioTestPage extends BaseAppPage {
  ...
  async runAudioCheck(expectedState, checkStereoTones = false) {
    ...
    try {
      ...
      this.logger.pushLogs(new Log(`Audio Check failed!! Error: \n ${e}`, LogLevel.ERROR));
    } finally {
      if (res) {
        res.logs.forEach(l => {
          this.logger.pushLogs(new Log(l));
        });
      }
    }
    if (!res) {
    ...
    for (let i = 0; i < res.percentages.length; i++) {
      this.logger.pushLogs(
        new Log(`Audio check success rate for channel ${i}: ${res.percentages[i] * 100}%`)
      );
    ...
    this.logger.pushLogs(new Log('Audio check passed!!'));
    return true;
  }
}
